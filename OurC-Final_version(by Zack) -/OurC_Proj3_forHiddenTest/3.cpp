...
bool gAfter_LS = false ; // LS '<<' 出現了代表下一個出現的ident或constant要印  
...
void Store_ALL_variable_name_and_value( int location, bool is_already_defined ) ;
...
void Assignment_calculator( int ident_location, int assignment_location ) { // assignment符號的計算
  ...
  if ( gALL_vari_Token[find_definition].detail_type == INT ) {
    ...
    if ( gToken[assignment_location].token_type == ONE_EQUAL ) { // '='
      ...
      gALL_vari_Token[find_definition].constant_value = 
      ( int )gALL_vari_Token[find_definition].constant_value % ( int )gValue[0] ;
    } // else if 
  } // if
    ...
    if ( gToken[assignment_location].token_type == ONE_EQUAL ) { // '='
      strcpy( gALL_vari_Token[find_definition].string_value, gToken[assignment_location+1].data ) ;
    } // if
  ...
  while ( i < gCout_Token.size() ) { 
    ...
    if ( gCout_Token[i].is_ident == true ) {
      ...
      if ( gALL_vari_Token[find_it].detail_type == INT ) {
        printf( "your variable is '%s' with the value use float is %f\n", 
                gALL_vari_Token[find_it].name , gALL_vari_Token[find_it].constant_value ) ;
        printf( "your variable is '%s' with the value use int is %d\n", 
                gALL_vari_Token[find_it].name , gALL_vari_Token[find_it].constant_value ) ;
        printf( "Statement executed ...\n" ) ;
      } // if
        ...
        printf( "your variable is '%s' with the value use string is %s\n", gALL_vari_Token[find_it].name , gALL_vari_Token[find_it].string_value ) ;
        printf( "Statement executed ...\n" ) ;
      } // else if
      else if ( gALL_vari_Token[find_it].detail_type == BOOL ) {
        if ( gALL_vari_Token[find_it].bool_value == true ) {
          printf( "your variable is '%s' with the value use bool is true\n" ) ;
          printf( "Statement executed ...\n" ) ;
        } // if
        if ( gALL_vari_Token[find_it].bool_value == false ) {
          printf( "your variable is '%s' with the value use bool is false\n" ) ;
          printf( "Statement executed ...\n" ) ;
        } // if
      } // else if

    } // if
    else if ( gCout_Token[i].is_ident == false ) {

      printf( "your constant with the value use float is %f\n", gCout_Token[i].constant_data ) ;
      printf( "Statement executed ...\n" ) ;
...
void Store_ALL_variable_name_and_value( int location, bool is_already_defined ) { // 存取變數資料進去vactor
  ...
  if ( !is_already_defined ) { // 沒有定義過就直接設定完 push back進去
    ...
    gALL_vari_Token[find].detail_type = gToken[location].detail_type ;
  } // else
          ...
          while ( i < gTemp_variable_loctation->size() ) { 
            ...
            if ( Definiton_and_New_Definition( i ) ) {
              ...
              if ( gToken[gTemp_variable_loctation->at( i )].is_funtion_defined == true ) {
                ...
                Store_ALL_variable_name_and_value( gTemp_variable_loctation->at( i ), false ) ; 
              } // else
    ...
    if ( gToken[gNum].token_type == CONSTANT ) {
      ...
      if ( gToken[gNum].detail_type == INT || gToken[gNum].detail_type == FLOAT ) {
        ...
        if ( !gIs_Definition && !gIs_function_Definition && !gIs_cout_operation ) {
          if ( gToken[gNum].detail_type == INT ) gValue.push_back( atoi( gToken[gNum].data ) ) ;
          else if ( gToken[gNum].detail_type == FLOAT ) gValue.push_back( atof( gToken[gNum].data ) ) ;
        } // if
...
bool Basic_expression() {
  ...
  if ( gToken[gNum].token_type == IDENT ) {
    ...
    if ( gIs_Definition ) {
      ...
      if ( gAfter_LS ) {
        gAfter_LS = false ;
        Temp_data_prepare_to_cout( gNum-1, true, 0 ) ; 
      } //if
...
bool Rest_of_maybe_mult_exp() { // 分號會到這個funtion然後直接return回去
  ...
  while ( gToken[gNum].token_type == MULTI || gToken[gNum].token_type == DIV 
    ...
    if ( gToken[gNum].token_type == MULTI ) {
      ...
      if ( Unary_exp() ) { // 應該是在這邊做計算
        if ( gValue.size() >= 2 ) {
          gTemp_value = ( int )gValue[gValue.size() - 2 ] % ( int )gValue[gValue.size() - 1] ;
          // 應該是倒數第二個加最後一個再繼續遞迴
          gValue.pop_back(); // 算完就丟掉
          gValue.pop_back(); // 算完就丟掉
          gValue.push_back( gTemp_value ); // 加入算好的值
...
bool Signed_unary_exp() {
  ...
  if ( gToken[gNum].token_type == IDENT ) {
    ...
    if ( gIs_Definition ) {
      ...
      if ( gToken[gNum-2].token_type == NEGATIVE ) { // 前面有接負號的話就去把他的值找出來乘負號
        int i = Find_variable_location_in_ALL_variables( gToken[gNum].data ) ;
        gALL_vari_Token[i].constant_value = ( -1 ) * gALL_vari_Token[i].constant_value ;
      } // if
      if ( gAfter_LS ) {
        gAfter_LS = false ;
        Temp_data_prepare_to_cout( gNum-1, true, 0 ) ;
  ...
  while ( find_id < gNum && !found_it ) { // 找找看有沒有這個變數
    ...
    if ( strcmp( gToken[find_id].data, gToken[gNum].data ) == 0
      ...
      if ( gALL_vari_Token[find].detail_type == INT || gALL_vari_Token[find].detail_type == FLOAT ) {
        gValue.push_back( gALL_vari_Token[find].constant_value ) ;
      } // if
      // 找到了就把值給取出來
