  ...
  while ( find_id < gNum && !found_it ) { // 找找看有沒有這個變數
    ...
    if ( strcmp( gToken[find_id].data, gToken[gNum].data ) == 0
      ...
      if ( find != -1 ) {
        ...
        if ( gALL_vari_Token[find].detail_type == INT || gALL_vari_Token[find].detail_type == FLOAT ) {
        ...
        else if ( gALL_vari_Token[find].detail_type == BOOL ) {
          ...
          if ( gALL_vari_Token[find].bool_value == true ) gBool_Ans.push_back( true ) ;
          else if ( gALL_vari_Token[find].bool_value == false ) gBool_Ans.push_back( false ) ;
        } // else if
        // 找到了就把值給取出來
