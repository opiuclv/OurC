...
void Assignment_calculator( int ident_location, int assignment_location ) { // assignment符號的計算
  ...
  if ( gALL_vari_Token[find_definition].detail_type == INT ) {
    ...
    if ( gIs_Bool_operation ) {
      ...
      gALL_vari_Token[find_definition].bool_value = gBool_Ans[0] ;
      gBool_Ans.clear() ;
    } // if
    else {
    ...
    if ( gToken[assignment_location].token_type == ONE_EQUAL ) { // '='
      ...
      strcat( gALL_vari_Token[find_definition].string_value, gString_value ) ;
    } // else if 
...
bool Boolean_operator() { // 看看是不是布林運算

  if ( gToken[gNum].token_type == EQUAL || gToken[gNum].token_type == NOT_EQUAL
       || gToken[gNum].token_type == BIGGER || gToken[gNum].token_type == BIGGGER_EQ
       || gToken[gNum].token_type == SMALLER || gToken[gNum].token_type == SMALLER_EQ ) return true ;
  else return false ; 
  ...
  if ( gToken[gNum].token_type == IDENT ) {
    ...
    if ( gIs_Definition ) {
      ...
      if ( gAfter_LS && gToken[gNum-2].token_type == S_LP && gToken[gNum].token_type != AND && 
           gToken[gNum].token_type != OR && !Boolean_operator() ) { 
        gAfter_LS = false ;
        char string_or_char[500] = "" ;
...
bool Rest_of_Identifier_started_basic_exp() {
  ...
  if ( gToken[gNum].token_type == M_LP || Assignment_operator() 
    ...
    if ( Assignment_operator() ) {
      ...
      strcpy( gPPMM_after_ID_info.name, gToken[gNum-1].data ) ;
...
bool Rest_of_maybe_logical_OR_exp() {
  ...
  while ( gToken[gNum].token_type == OR ) {
    ...
    if ( find1 != -1 && gALL_vari_Token[find1].detail_type == BOOL ) {
    ...
    if ( Maybe_logical_AND_exp() ) {
      ...
      if ( gBool_Ans.size() > 0 ) { // 代表後面的算式 是一連串的的布林運算
        ...
        if ( gALL_vari_Token[find2].detail_type == BOOL ) { // 這個ident是布林就把值撈出來
  ...
  while ( gToken[gNum].token_type == LS || gToken[gNum].token_type == RS ) {
    ...
    if ( gIn_expresstion ) {
      ...
      if ( gToken[gNum-1].token_type == LS ) {
        ...
        int count = 0 ;
        int letf_shift = 1 ;
        while ( count < atoi( gToken[gNum].data ) ) {
          letf_shift = letf_shift * 2 ;
          count ++ ;
        } // while

        gValue[gValue.size()-1] = gValue[gValue.size()-1] * letf_shift ;
        ...
        gValue[gValue.size()-1] = gValue[gValue.size()-1] / right_shift ;
...
bool Undefined_error() {
  ...
  while ( find_id < gNum && !found_it ) { // 找找看有沒有這個變數
    ...
    if ( strcmp( gToken[find_id].data, gToken[gNum].data ) == 0
      ...
      else if ( gALL_vari_Token[find].detail_type == STRING || gALL_vari_Token[find].detail_type == CHAR ) {
