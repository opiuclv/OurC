...
bool Command() { // 回傳false 代表結束程式 回傳true代表還要繼續執行
  ...
  if ( gToken[gNum].token_type == IDENT ) {  // 第一種情況   以IDENT開頭的句子
    ...
    else if ( !gHave_error && gToken[gNum].token_type == UNEXPECTED ) {
      // a. ; 錯在'.'
      // 直接return就好
      // /////////////////////////////////////////////////////////////////////////////////////////////////
      Unexpected_error() ;
...
bool IDlessArithExpOrBexp() {
  ...
  printf( "-------- into IDlessArithExpOrBexp() '%s', token_type '%d'\n", gToken[gNum].data, gToken[gNum].token_type ) ;
  ...
  else if ( !gHave_error || gToken[gNum].token_type == QUIT ) {
    // ex . a quit ; 錯在'quit'
    // ///////////////////////////////////////////////////////////////////////////////////////////////////
    Unexpected_error() ;
    // ///////////////////////////////////////////////////////////////////////////////////////////////////
...
bool BooleanOprator() {
  ...
  printf( "-------- into BooleanOprator() '%s', token_type '%d'\n", gToken[gNum].data, gToken[gNum].token_type ) ;
...
bool NOT_ID_StartArithExpOrBexp() {
  ...
  printf( "-------- into NOT_ID_StartArithExpOrBexp() '%s', token_type '%d'\n", gToken[gNum].data, gToken[gNum].token_type ) ;
  ...
  if ( gToken[gNum].token_type == PLUS || gToken[gNum].token_type == MINUS ) {
    ...
    else if ( gToken[gNum].token_type == INT ) {
...
bool NOT_ID_StartArithExp() {
  ...
  printf( "-------- into NOT_ID_StartArithExp() '%s', token_type '%d'\n", gToken[gNum].data, gToken[gNum].token_type ) ;
...
bool NOT_ID_StartTerm() {
  ...
  printf( "-------- into NOT_ID_StartTerm() '%s', token_type '%d'\n", gToken[gNum].data, gToken[gNum].token_type ) ;
...
bool NOT_ID_StartFactor() {
  ...
  printf( "-------- into NOT_ID_StartFactor() '%s', token_type '%d'\n", gToken[gNum].data, gToken[gNum].token_type ) ;
...
bool ArithExp() {
  ...
  printf( "-------- into ArithExp() '%s', token_type '%d'\n", gToken[gNum].data, gToken[gNum].token_type ) ;
  if ( !Term() ) {
    // /////////////////////////////////////////////////////////////////////////////////////////////
    if ( !gHave_error ) Unexpected_error() ;
    // /////////////////////////////////////////////////////////////////////////////////////////////
  ...
  printf( "-------- into ArithExp2() '%s', token_type '%d'\n", gToken[gNum].data, gToken[gNum].token_type ) ;
...
bool Term() {
  printf( "-------- into Term() '%s', token_type '%d'\n", gToken[gNum].data, gToken[gNum].token_type ) ;
  ...
  printf( "-------- into Term2() '%s', token_type '%d'\n", gToken[gNum].data, gToken[gNum].token_type ) ;
...
bool Factor() {
  ...
  printf( "-------- into Factor() '%s', token_type '%d'\n", gToken[gNum].data, gToken[gNum].token_type ) ;
  // factor判斷不應該peekToken
...
bool GetToken( bool get_error ) {
  ...
  if ( ch[0] == '+' || ch[0] == '*' || ch[0] == '-' ) { // 處理加乘
    ...
    printf( "-------- GetToken data:'%s' token_type:'%d' token_line:'%d'\n"
        , gToken[gToken.size()-1].data, gToken[gToken.size()-1].token_type, gToken[gToken.size()-1].lineNum ) ;
    return true ;
  ...
  if ( ch[0] == 'q' ) { // quit 的處理
    ...
    if ( strcmp( text, "quit" ) == 0 && !not_quit ) { // 確認是不是讀到quit
      ...
      if ( get_error == false ) return false ;
  ...
  if ( ch[0] != ' ' && 29 < ch[0] && ch[0] < 40 ) { // 不是任何東西 ex. $$
  ...
  if ( ch[0] != ' ' && 90 < ch[0] && ch[0] < 97 && ch[0] < 95 ) {
        // 不是任何東西 ex. [2
...
int main() {
  ...
  while ( GetToken( gGet_error ) || gHave_error ) {
    ...
    printf( "-------- Into by '%s', gToken.size == '%d', gToken.type == '%d'\n", gToken[gNum].data, gToken.size(), gToken[gNum].token_type ) ;
    Command() ;
