...
void IDlessArithExpOrBexp() {
  ...
  while ( find_id < gNum && !found_it ) { // 找找看有沒有這個變數
    ...
    if ( strcmp( gToken[find_id].data, gToken[gNum].data ) == 0 && gToken[find_id].have_defined == true ) {
      ...
      found_it = true ;
      if ( !gToken[find_id].is_int ) gFloat_operation = true ; // 這個變數不是int 是 float
    } // if
...
void NOT_ID_StartArithExp() {
  ...
  if ( gToken.size() - gNum > 1 ) { // 確保不會沒有空間還加
    ...
    if ( BooleanOprator() || gToken[gNum+1].token_type == SEMICOLON || gHave_error ) {
      return;
    } // if
    else if ( !gHave_error && gToken[gNum+1].token_type == UNRECOGNIZED ) {
...
void NOT_ID_StartTerm() {
  ...
  if ( gToken.size() - gNum > 1 ) { // 確保不會沒有空間還加
    ...
    if ( gToken[gNum+1].token_type == PLUS || gToken[gNum+1].token_type == MINUS
         || BooleanOprator()
         || gToken[gNum+1].token_type == SEMICOLON || gHave_error ) {
      return;
    } // if
    else if ( !gHave_error && gToken[gNum+1].token_type == UNRECOGNIZED ) {
...
void Factor()
{
  ...
  else if ( gToken[gNum].token_type == QUIT && !gHave_error ) { // 如果沒有錯 可是遇到quit了代表這是一個未定義的ident
...
// 以quit為讀取一次的終止點
// 以分號為讀取一行的分隔點
bool GetToken() { // GetToken 先全部照單全收，要印的時候用C語言把歸類成 float的token，用%3f的函數去印

  // 宣吿
  char text[500] = "" ; // 用來暫存token
  char ch[500] = "" ; // 用來讀檔
  char temp_ch[500] = "" ; // 輔助讀檔
  char temp = ' ' ;
  char for_unre[500] = "" ; // 給unrecognized用的
  int line = 0 ;
...
int main() {
  ...
  while ( gToken[gNum].token_type != QUIT ) {
