  ...
  while ( temp != EOF ) { // 讀到結束
    ...
    while ( temp != EOF && gToken_info.token_type != QUIT ) { // 讀到換行 \n 還不知道老大input怎麼給 先不動
      ...
      if ( 58 > ch[0] && ch[0] > 47 && !for_break ) { // 數字開頭的處理
        ...
        if ( is_sign ) {
          is_sign = false ;
          strcat( text, "-" ) ; // 到了這裡is sign居然還有作用代表 可能是這樣的情況需要補他的符號
        } // if
      ...
      if ( !for_break && ch[0] != ' ' && 90 < ch[0] && ch[0] < 97 && ch[0] != 92 && ch[0] != 95 ) { 
           // 不是任何東西 ex. [2
      ...
      if ( !for_break && ch[0] != ' ' && 122 < ch[0] && ch[0] < 127 ) { 
           // 不是任何東西 ex. {A
