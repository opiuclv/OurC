...
bool gHave_negative = false ; // 特殊功能 for 1--(36) = 37 而不是 -35 [括號外面!!]
bool gHave_E_mark = false ; // 特殊功能 for b || !(b1 && b2) 
bool gHave_E_mark_after_sLP = false ; // for ( !b1 || b1 ) 的情況
...
void Assignment_calculator( int ident_location, int assignment_location ) { // assignment符號的計算
  ...
  if ( find_definition != -1 ) {
    ...
    if ( gALL_vari_Token[find_definition].detail_type == INT ) {
      ...
      if ( gIs_Bool_operation && gBool_Ans.size() > 0 ) {
        // gIs_Bool_operation = false ;
        gALL_vari_Token[find_definition].bool_value = gBool_Ans[gBool_Ans.size() - 1] ;
      } // if
      else if ( gBool_Ans.size() == 0 && gToken[assignment_location].token_type == ONE_EQUAL ) {
...
bool Basic_expression() {
  ...
  if ( gToken[gNum].token_type == IDENT ) {
    ...
    if ( gIs_Definition ) {
      ...
      // 這個if only for b = b1 ; 或是 cout << ( b1 = b2 ) ; 這種後面只有一個"布林"變數賦值不會存入gBool_Ans的情況
      int find = -1 ;
      find = Find_variable_location_in_ALL_variables( gToken[gNum-1].data ) ;
      gNum = gNum-2 ;
      if ( find != -1 && Assignment_operator() && gALL_vari_Token[find].detail_type == BOOL ) {

        gNum = gNum + 2 ;
        if ( gToken[gNum].token_type == SEMICOLON || gALL_vari_Token[find].detail_type == LS || 
             gALL_vari_Token[find].detail_type == S_RP ) {
          gBool_Ans.push_back( gALL_vari_Token[find].bool_value ) ;
        } // if
...
bool Rest_of_maybe_logical_OR_exp() {
  ...
  while ( gToken[gNum].token_type == OR ) {
    ...
    if ( gToken[gNum-2].token_type != S_LP && !Boolean_operator( gNum-2 ) && gBool_Ans.size() > 0 ) {
      ...
      if ( gHave_E_mark_after_sLP != true ) {
      ...
      if ( gBool_Ans.size() > 0 ) { // 代表後面的算式 是一連串的的布林運算
        value1 = gBool_Ans[gBool_Ans.size()-1] ;
        gBool_Ans.pop_back() ;
      } // if
      else if ( find1 != -1 && gALL_vari_Token[find1].detail_type == CHAR ) {
        char1 =  gALL_vari_Token[find1].string_value[0] ;
        have_char1 = true ;
      } // else if
      else if ( gToken[gNum-1].detail_type == CHAR ) {
        char1 =  gToken[gNum-1].data[0] ;
        have_char1 = true ;
      } // else if
    ...
    if ( Maybe_logical_AND_exp() ) {
      ...
      // 或是 a && b && c 都是布林
      int find2 = -1 ;
      if ( gToken[temp_gNum+1].token_type == IDENT ) {
      ...
      if ( find2 != -1 && gALL_vari_Token[find2].detail_type == BOOL ) { // 這個ident是布林就把值撈出來
        ...
        else if ( find1 != -1 && gALL_vari_Token[find2].detail_type == CHAR ) {
          char2 =  gALL_vari_Token[find2].string_value[0] ;
          have_char2 = true ;
        } // else if
        else if ( gToken[gNum-1].detail_type == CHAR ) {
          char2 =  gToken[gNum-1].data[0] ;
          have_char2 = true ;
        } // else if
        else if ( gValue.size() > 0 ) { // 他是其他計算
      ...
      if ( !have_char1 && !have_char2 ) {
        if ( value1 || value2 ) {
          if ( gBool_Ans.size() > 0 ) gBool_Ans.pop_back() ;
...
bool Maybe_equality_exp() {
  ...
  while ( gToken[gNum].token_type == EQUAL || gToken[gNum].token_type == NOT_EQUAL ) {
    ...
    else if ( gToken[gNum-2].token_type == IDENT ) {

      int find = Find_variable_location_in_ALL_variables( gToken[gNum-2].data ) ;
      if ( gALL_vari_Token[find].detail_type == CHAR ) {
        char1 =  gALL_vari_Token[find].string_value[0] ;
        have_char1 = true ;
      } // if
      else if ( gALL_vari_Token[find].detail_type == STRING ) {
        strcpy( string1, gString_value ) ;
        strcpy( gString_value, "" ) ; // 比較前存好了 就要初始化
        have_string = true ;
      } // else if

    } // else if
    else if ( gToken[gNum-2].detail_type == CHAR ) {
      char1 =  gToken[gNum-2].data[0] ;
      have_char1 = true ;
    } // else if
    else if ( gToken[gNum-2].detail_type == STRING ) {
      strcpy( string1, gString_value ) ;
      strcpy( gString_value, "" ) ; // 比較前存好了 就要初始化
      have_string = true ;
    } // else if
    ...
    if ( Maybe_relational_exp() ) {
      ...
      else if ( gToken[gNum-1].token_type == IDENT ) {

        int find = Find_variable_location_in_ALL_variables( gToken[gNum-1].data ) ;
        if ( gALL_vari_Token[find].detail_type == CHAR ) {
          char2 =  gALL_vari_Token[find].string_value[0] ;
          have_char2 = true ;
        } // if
        else if ( gALL_vari_Token[find].detail_type == STRING ) {
          strcpy( string2, gString_value ) ;
          strcpy( gString_value, "" ) ; // 比較前存好了 就要初始化
          have_string = true ;
        } // else if
      ...
      else if ( gToken[gNum-1].detail_type == STRING ) {
        strcpy( string2, gString_value ) ;
        strcpy( gString_value, "" ) ; // 比較前存好了 就要初始化
        have_string = true ;
      } // else if
      ...
      if ( !have_char1 && !have_char2 && !have_string ) {
        ...
        if ( gToken[temp_gNum].token_type == EQUAL && char1 == value2
             && !gHave_error ) {
          gBool_Ans.push_back( true ) ;
        } // if
        else if ( gToken[temp_gNum].token_type == EQUAL && !gHave_error ) {
          if ( char1 != value2 ) gBool_Ans.push_back( false ) ;
        } // else if
        else if ( gToken[temp_gNum].token_type == NOT_EQUAL && !gHave_error ) {
        ...
        if ( gToken[temp_gNum].token_type == EQUAL && value1 == char2
             && !gHave_error ) {
          gBool_Ans.push_back( true ) ;
        } // if
        else if ( gToken[temp_gNum].token_type == EQUAL && !gHave_error ) {
          if ( value1 != char2 ) gBool_Ans.push_back( false ) ;
        } // else if
        else if ( gToken[temp_gNum].token_type == NOT_EQUAL && !gHave_error ) {
        ...
        if ( gToken[temp_gNum].token_type == EQUAL && char1 == char2
             && !gHave_error ) {
          gBool_Ans.push_back( true ) ;
        } // if
        else if ( gToken[temp_gNum].token_type == EQUAL && !gHave_error ) {
          if ( char1 != char2 ) gBool_Ans.push_back( false ) ;
        } // else if
        else if ( gToken[temp_gNum].token_type == NOT_EQUAL && !gHave_error ) {
      ...
      else if ( !have_char1 && !have_char2 && have_string ) {


        if ( gToken[temp_gNum].token_type == EQUAL && strcmp( string1, string2 ) == 0
             && !gHave_error ) {
          gBool_Ans.push_back( true ) ;
        } // if
        else if ( gToken[temp_gNum].token_type == EQUAL && !gHave_error ) {
          if ( strcmp( string1, string2 ) != 0 ) gBool_Ans.push_back( false ) ;
        } // else if
        else if ( gToken[temp_gNum].token_type == NOT_EQUAL && !gHave_error ) {
...
bool Maybe_relational_exp() {
  ...
  while ( gToken[gNum].token_type == SMALLER_EQ || gToken[gNum].token_type == BIGGGER_EQ
    ...
    else if ( gToken[gNum-2].detail_type == TRUE || gToken[gNum-2].detail_type == FALSE ) {

      if ( gToken[gNum-2].detail_type == TRUE ) {
        value1 = 1 ;
      } // if
      else if ( gToken[gNum-2].detail_type == FALSE ) {
        value1 = 0 ;
      } // else if
    ...
    if ( Maybe_shift_exp() ) {
      ...
      if ( gValue.size() > 0 ) value2 = gValue[gValue.size()-1] ; // 布林後面的計算再存起來
        ...
        if ( gToken[gNum-1].detail_type == TRUE ) {
          value2 = 1 ;
        } // if
        else if ( gToken[gNum-2].detail_type == FALSE ) {
          value2 = 0 ;
      ...
      if ( !have_char1 && !have_char2 && !have_string ) {
        ...
        if ( gToken[temp_gNum].token_type == BIGGER && char1 > value2 && !gHave_error ) {
          gBool_Ans.push_back( true ) ;
        } // if
        else if ( gToken[temp_gNum].token_type == BIGGER && char1 <= value2 && !gHave_error  ) {
          gBool_Ans.push_back( false ) ;
        } // else if
        else if ( gToken[temp_gNum].token_type == BIGGGER_EQ && char1 >= value2 && !gHave_error  ) {
          gBool_Ans.push_back( true ) ;
        } // else if
        else if ( gToken[temp_gNum].token_type == BIGGGER_EQ && char1 < value2 && !gHave_error  ) {
          gBool_Ans.push_back( false ) ;
        } // else if
        else if ( gToken[temp_gNum].token_type == SMALLER && char1 < value2 && !gHave_error  ) {
          gBool_Ans.push_back( true ) ;
        } // else if
        else if ( gToken[temp_gNum].token_type == SMALLER && char1 >= value2 && !gHave_error  ) {
          gBool_Ans.push_back( false ) ;
        } // else if
        else if ( gToken[temp_gNum].token_type == SMALLER_EQ && char1 <= value2 && !gHave_error  ) {
          gBool_Ans.push_back( true ) ;
        } // else if
        else if ( gToken[temp_gNum].token_type == SMALLER_EQ && char1 > value2 && !gHave_error  ) {
          gBool_Ans.push_back( false ) ;
        ...
        if ( gToken[temp_gNum].token_type == BIGGER && value1 > char2 && !gHave_error ) {
          gBool_Ans.push_back( true ) ;
        } // if
        else if ( gToken[temp_gNum].token_type == BIGGER && value1 <= char2 && !gHave_error  ) {
          gBool_Ans.push_back( false ) ;
        } // else if
        else if ( gToken[temp_gNum].token_type == BIGGGER_EQ && value1 >= char2 && !gHave_error  ) {
          gBool_Ans.push_back( true ) ;
        } // else if
        else if ( gToken[temp_gNum].token_type == BIGGGER_EQ && value1 < char2 && !gHave_error  ) {
          gBool_Ans.push_back( false ) ;
        } // else if
        else if ( gToken[temp_gNum].token_type == SMALLER && value1 < char2 && !gHave_error  ) {
          gBool_Ans.push_back( true ) ;
        } // else if
        else if ( gToken[temp_gNum].token_type == SMALLER && value1 >= char2 && !gHave_error  ) {
          gBool_Ans.push_back( false ) ;
        } // else if
        else if ( gToken[temp_gNum].token_type == SMALLER_EQ && value1 <= char2 && !gHave_error  ) {
          gBool_Ans.push_back( true ) ;
        } // else if
        else if ( gToken[temp_gNum].token_type == SMALLER_EQ && value1 > char2 && !gHave_error  ) {
          gBool_Ans.push_back( false ) ;
        ...
        if ( gToken[temp_gNum].token_type == BIGGER && char1 > char2 && !gHave_error ) {
          gBool_Ans.push_back( true ) ;
        } // if
        else if ( gToken[temp_gNum].token_type == BIGGER && char1 <= char2 && !gHave_error  ) {
          gBool_Ans.push_back( false ) ;
        } // else if
        else if ( gToken[temp_gNum].token_type == BIGGGER_EQ && char1 >= char2 && !gHave_error  ) {
          gBool_Ans.push_back( true ) ;
        } // else if
        else if ( gToken[temp_gNum].token_type == BIGGGER_EQ && char1 < char2 && !gHave_error  ) {
          gBool_Ans.push_back( false ) ;
        } // else if
        else if ( gToken[temp_gNum].token_type == SMALLER && char1 < char2 && !gHave_error  ) {
          gBool_Ans.push_back( true ) ;
        } // else if
        else if ( gToken[temp_gNum].token_type == SMALLER && char1 >= char2 && !gHave_error  ) {
          gBool_Ans.push_back( false ) ;
        } // else if
        else if ( gToken[temp_gNum].token_type == SMALLER_EQ && char1 <= char2 && !gHave_error  ) {
          gBool_Ans.push_back( true ) ;
        } // else if
        else if ( gToken[temp_gNum].token_type == SMALLER_EQ && char1 > char2 && !gHave_error  ) {
          gBool_Ans.push_back( false ) ;
        ...
        if ( gToken[temp_gNum].token_type == BIGGER && strcmp( string1, string2 ) > 0 && !gHave_error ) {
          gBool_Ans.push_back( true ) ;
        } // if
        else if ( gToken[temp_gNum].token_type == BIGGER && strcmp( string1, string2 ) <= 0 && 
                  !gHave_error  ) {
          gBool_Ans.push_back( false ) ;
        } // else if
        else if ( gToken[temp_gNum].token_type == BIGGGER_EQ && strcmp( string1, string2 ) >= 0 && 
                  !gHave_error  ) {
          gBool_Ans.push_back( true ) ;
        } // else if
        else if ( gToken[temp_gNum].token_type == BIGGGER_EQ && strcmp( string1, string2 ) < 0 && 
        ...
        else if ( gToken[temp_gNum].token_type == SMALLER && strcmp( string1, string2 ) < 0 && 
                  !gHave_error  ) {
          gBool_Ans.push_back( true ) ;
        } // else if
        else if ( gToken[temp_gNum].token_type == SMALLER && strcmp( string1, string2 ) >= 0 && 
        ...
        else if ( gToken[temp_gNum].token_type == SMALLER_EQ && strcmp( string1, string2 ) <= 0 && 
                  !gHave_error  ) {
          gBool_Ans.push_back( true ) ;
        } // else if
        else if ( gToken[temp_gNum].token_type == SMALLER_EQ && strcmp( string1, string2 ) > 0 && 
  ...
  while ( gToken[gNum].token_type == SMALLER_EQ || gToken[gNum].token_type == BIGGGER_EQ
    ...
    if ( Maybe_shift_exp() ) {
      ...
      if ( gValue.size() > 0 ) value2 = gValue[gValue.size()-1] ; // 布林後面的計算再存起來
        ...
        else if ( gToken[gNum-1].detail_type == FALSE ) {
          value2 = 0 ;
        } // else if
      ...
      if ( !have_char1 && !have_char2 && !have_string ) {
        ...
        if ( gToken[temp_gNum].token_type == BIGGER && strcmp( string1, string2 ) > 0 && 
             !gHave_error ) {
          gBool_Ans.push_back( true ) ;
        } // if
        else if ( gToken[temp_gNum].token_type == BIGGER && strcmp( string1, string2 ) <= 0 && 
...
bool Maybe_shift_exp() {
  ...
  while ( gToken[gNum].token_type == LS || gToken[gNum].token_type == RS ) {
    ...
    if ( Maybe_additive_exp() ) {
      ...
      if ( gIs_Bool_operation && !gIdent_operation ) {
        gIs_Bool_operation = false ;
        char string_or_char[500] = "" ;
        Temp_data_prepare_to_cout( 0, false, false, false, false, true, 
                                   0, gBool_Ans[gBool_Ans.size() - 1], string_or_char ) ;
...
bool Maybe_additive_exp() {
  ...
  while ( gToken[gNum].token_type == PLUS || gToken[gNum].token_type == SUB ) {
    ...
    if ( gToken[gNum-1].detail_type == TRUE || gToken[gNum-1].detail_type == FALSE ) {
    ...
    else if ( gValue.size() > 0 ) {
      value1 = gValue[gValue.size() - 1] ;
    } // else if
    else if ( gIs_string_operation ) ; // string的加法會自動加
    else if ( gToken[gNum-1].token_type == IDENT ) { 
      int find = -1 ;
      find = Find_variable_location_in_ALL_variables( gToken[gNum-1].data ) ;
      if ( find != -1 && gALL_vari_Token[find].detail_type == STRING ) ;
      else if ( find != -1 && gALL_vari_Token[find].detail_type == CHAR ) ;
      else return false ;
    } // else if 
    else if ( gToken[gNum-1].detail_type == STRING ) ;
    else if ( gToken[gNum-1].detail_type == CHAR ) ;
    else if ( gToken[gNum-1].token_type == S_RP ) ;
    else if ( gToken[gNum-1].token_type == M_RP ) {
      int find = -1 ;
      find = Find_variable_location_in_ALL_variables( gToken[gNum-4].data ) ;
      if ( find != -1 && gALL_vari_Token[find].detail_type == STRING ) ;
      else if ( find != -1 && gALL_vari_Token[find].detail_type == CHAR ) ;
      else return false ;
    } // else if
    ...
    if ( gToken[gNum].token_type == PLUS ) {
      ...
      if ( Maybe_mult_exp() ) { // 應該是在這邊做計算
        ...
        if ( gToken[gNum].detail_type == TRUE || gToken[gNum].detail_type == FALSE ) {

          if ( gToken[gNum].detail_type == TRUE ) {
            value2 = 1 ;
          } // if
          else if ( gToken[gNum].detail_type == FALSE ) {
            value2 = 0 ;
          } // else if

        } // if
        else if ( gValue.size() > 0 ) {
          value2 = gValue[gValue.size() - 1] ;
        } // else if
        else if ( gIs_string_operation ) ; // string的加法會自動加
        else if ( gToken[gNum-1].token_type == IDENT ) { 
          int find = -1 ;
        ...
        else gTemp_value = value1 + value2;
        // 應該是倒數第二個加最後一個再繼續遞迴
...
bool Signed_unary_exp() {
  ...
  if ( gToken[gNum].token_type == IDENT ) {
    ...
    if ( gToken[gNum-2].detail_type == E_MARK ) {
      ...
      if ( find != -1 ) {
        if ( gALL_vari_Token[find].detail_type == BOOL ) {
          if ( gALL_vari_Token[find].bool_value == true )  gBool_Ans.push_back( false ) ;
          else if ( gALL_vari_Token[find].bool_value == false )  gBool_Ans.push_back( true ) ;
        } // if 
      } // if
    ...
    // for ( !true || !true ) 的情況
    if ( gToken[gNum-1].detail_type == E_MARK ) gHave_E_mark_after_sLP = true ;
    ...
    if ( gIs_Definition ) {
      ...
      if ( gAfter_LS ) {
        ...
        if ( find != -1 ) {
          if ( gALL_vari_Token[find].detail_type == INT || gALL_vari_Token[find].detail_type == FLOAT ||
               gALL_vari_Token[find].detail_type == BOOL ) {
            if ( gToken[gNum].token_type == LS ) { // cout << a << "\n" ; 箭頭中間只有一個變數
              gAfter_LS = false ;
              char string_or_char[500] = "" ;
              Temp_data_prepare_to_cout( gNum-1, true, false, false, false, false, 
                                         0, false, string_or_char ) ; 
            } // if
            else gIdent_operation = true ; // 變數要繼續計算
          } // if
          else if ( gALL_vari_Token[find].detail_type == STRING || 
                    gALL_vari_Token[find].detail_type == CHAR ) {
            if ( gToken[gNum].token_type == LS ) { // cout << a << "\n" ; 箭頭中間只有一個變數
            ...
            else gIs_string_operation = true ;
          } // else if
